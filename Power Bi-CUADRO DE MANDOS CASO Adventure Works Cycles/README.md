...
* **Beneficio Absoluto (`Profit`):**
  ```dax
  Profit = SUM(Fact_Sales[Sales_Amount]) - SUM(Fact_Sales[Cost_of_Goods_Sold])
  ```

* **Puntuación Media (Radar Chart):**
  ```dax
  Puntuacion Media = AVERAGE(Fact_Sales[Order_Quantity])
  ```

* **Meta Presupuestada (Benchmark de Radar):**
  ```dax
  Meta Presupuestada = CALCULATE(
      [Puntuacion Media],
      REMOVEFILTERS(Dim_Product[Product_Category])
  ) * 1.10
  ```

* **Ingresos Escenario Base (Tornado Chart - Lado Izquierdo):**
  ```dax
  Ingresos Escenario Base = CALCULATE(
      SUM(Fact_Sales[Sales_Amount]),
      Fact_Sales[Scenario] = "Base"
  )
  ```

* **Ingresos Escenario Optimista (Tornado Chart - Lado Derecho):**
  ```dax
  Ingresos Escenario Optimista = CALCULATE(
      SUM(Fact_Sales[Sales_Amount]),
      Fact_Sales[Scenario] = "Optimista"
  )
  ```
